...
class PhysicsEngine:
    ...
    @staticmethod
    def generate_temperature_profile(t, seed=1234):
        """生成更接近工程场景的复合环境温度曲线

        组成：
        1) 轨道周期项：模拟进出日照区导致的主周期变化；
        2) 慢漂移项：模拟平台热平衡点随时间缓慢偏移；
        3) 载荷热事件：模拟设备开关机或任务窗口引起的局部升温；
        4) 有色随机项：模拟热环境中的低频随机波动，而非理想白噪声；
        5) 测量噪声：模拟温度测量链路的小幅读数噪声。
        """
        ...
        # 1) 轨道主周期项：这里取 12 小时近似周期，叠加次谐波，避免单一正弦过于规则
        ...
        # 2) 慢漂移项：模拟热平衡点缓慢变化（48 小时量级）
        slow_period = 48.0 * 3600.0
        T_slow = 0.8 * np.sin(2 * np.pi * t / slow_period + 0.7)

        # 3) 随机游走型缓慢漂移（极低频）
        drift_steps = rng.normal(0.0, 7e-5 * np.sqrt(dt), N)
        T_drift = np.cumsum(drift_steps)
        T_drift = np.clip(T_drift, -0.5, 0.5)

        # 4) 热惯性背景扰动：一阶 AR(1) 有色噪声，体现温度变化连续性
        rho = np.exp(-dt / 1800.0)  # 约 30 min 相关时间
        thermal_colored = np.zeros(N)
        eps = rng.normal(0.0, 0.015, N)
        for k in range(1, N):
            thermal_colored[k] = rho * thermal_colored[k - 1] + np.sqrt(1.0 - rho ** 2) * eps[k]

        # 5) 载荷热事件：用平滑阶跃表示任务开始/结束导致的升温，再自然回落
        def smooth_pulse(t_axis, t_on, duration, amp, tau_edge):
        ...
        if total_hours >= 8:
            T_event += smooth_pulse(t, 6.0 * 3600.0, 2.0 * 3600.0, 0.9, 250.0)
        if total_hours >= 20:
            T_event += smooth_pulse(t, 16.0 * 3600.0, 3.5 * 3600.0, -0.7, 320.0)
        if total_hours >= 32:
            T_event += smooth_pulse(t, 28.0 * 3600.0, 2.5 * 3600.0, 1.2, 280.0)
        if total_hours >= 44:
            T_event += smooth_pulse(t, 40.0 * 3600.0, 2.0 * 3600.0, -0.5, 260.0)

        # 6) 小幅测量噪声
        T_meas_noise = rng.normal(0.0, 0.01, N)

        return T0 + T_orbit + T_slow + T_drift + thermal_colored + T_event + T_meas_noise
